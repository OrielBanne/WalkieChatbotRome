...
def render_itinerary_summary(itinerary: Itinerary):
    ...
    with col1:
        st.metric("Total Duration", f"{itinerary.total_duration_minutes // 60}h {itinerary.total_duration_minutes % 60}m")
    with col2:
        st.metric("Walking Distance", f"{itinerary.total_distance_km:.1f} km")
    with col3:
        st.metric("Total Cost", f"€{itinerary.total_cost:.2f}")
    with col4:
        ...
        if score >= 80:
            ...
            emoji = "❌"
        st.metric("Feasibility", f"{emoji} {score:.0f}/100")
...
def render_itinerary_map(itinerary: Itinerary, discovered_places=None):
    ...
    mode_option = st.radio(
        ...
        horizontal=True, index=0, key="itinerary_transport_mode"
    )
    mode_map = {
        "Auto (Smart)": None, "🚶 Walking": "pedestrian",
        "🚗 Driving": "car", "🚌 Public Transport": "public_transport"
    }
    ...
    cache_parts = []
    for m in markers:
        cache_parts.append(f"{m.name}:{m.coordinates}")
    if discovered_places:
        for dp in discovered_places:
            cache_parts.append(f"d:{dp.name}:{dp.coordinates}")
    cache_parts.append(f"mode:{transport_mode}")
    map_cache_key = "|".join(cache_parts)
    
    if (st.session_state.get("_map_cache_key") == map_cache_key
            and "_map_cache_obj" in st.session_state
            and "_map_cache_seen_types" in st.session_state):
        map_obj = st.session_state["_map_cache_obj"]
        seen_types = st.session_state["_map_cache_seen_types"]
    else:
        map_builder = MapBuilder()
        map_obj = map_builder.create_map_with_places(
            places=markers, add_route=True, transport_mode=transport_mode,
            show_center_marker=False, numbered_markers=True
        )
        from folium.plugins import LocateControl
        LocateControl(
            auto_start=False,
            strings={"title": "Show my location", "popup": "You are here"},
            flyTo=True, keepCurrentZoomLevel=True
        ).add_to(map_obj)
        ...
        route_mode_labels = {
            None: ("Auto route", "#2A81CB"), "pedestrian": ("Walking route", "#2A81CB"),
            "car": ("Driving route", "#CB2B3E"), "public_transport": ("Public transport", "#2AAD27"),
        }
        ...
        if legend_entries:
            ...
            <div style="
                ...
                background: rgba(255,255,255,0.92); border: 1px solid #aaa;
                border-radius: 5px; padding: 6px 10px; z-index: 9999;
                font-family: Arial, sans-serif; box-shadow: 0 1px 4px rgba(0,0,0,0.2);
            ">
        ...
        # Cache the map object for reuse
    ...
    # Legend below the map (always visible)
    route_mode_labels = {
        None: ("Auto route", "#2A81CB"), "pedestrian": ("Walking route", "#2A81CB"),
        "car": ("Driving route", "#CB2B3E"), "public_transport": ("Public transport", "#2AAD27"),
    }
    ...
    for ptype in seen_types:
        ...
        if label not in seen_labels:
            seen_labels.add(label)
            dot = f'<span style="display:inline-block;width:11px;height:11px;border-radius:50%;background:{css_color};margin-right:3px;vertical-align:middle;border:1px solid #999;"></span>'
...
def render_itinerary(itinerary: Optional[Itinerary], discovered_places=None):
    """Legacy wrapper — calls render_itinerary_content. Kept for backward compatibility."""
    render_itinerary_content(itinerary, discovered_places=discovered_places)


def render_itinerary_content(itinerary: Optional[Itinerary], discovered_places=None):
    """
    Render itinerary content for the left column (everything except the map).
    ...
    if stop_removed and removed_index is not None:
        st.session_state.itinerary_action = {"type": "remove", "index": removed_index}
        st.rerun()
    
    # Discovered places checkboxes
    if discovered_places:
        st.markdown("---")
        st.markdown("#### ⭐ Discovered Places")
        selected_to_add = []
        col_btn, col_places = st.columns([1, 4])
        with col_places:
            for dp in discovered_places:
        ...
        with col_btn:
            ...
            if selected_to_add:
                ...
                if total_after > 7:
                    st.caption(f"⚠️ {total_after} stops")
                if st.button("➕ Add", key="add_discovered_btn"):
                    st.session_state.itinerary_action = {
                        "type": "add_multiple", "place_names": selected_to_add
                    }
                    st.rerun()
    ...
    with col1:
        new_place_name = st.text_input(
            "Place name", placeholder="e.g., Trevi Fountain, Pantheon, etc.",
            key="new_place_input"
        )
    with col2:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("➕ Add Stop", use_container_width=True, disabled=not new_place_name):
            st.session_state.itinerary_action = {"type": "add", "place_name": new_place_name.strip()}
            st.rerun()
...
def generate_text_itinerary(itinerary: Itinerary) -> str:
    ...
    for i, stop in enumerate(itinerary.stops, 1):
        ...
        if stop.crowd_level:
            crowd_labels = {
                CrowdLevel.LOW: "Low", CrowdLevel.MEDIUM: "Medium",
        ...
        lines.append("")
    if itinerary.explanation:
        lines.extend(["", "PLANNING NOTES", "-" * 60, itinerary.explanation, ""])
    lines.extend(["", "=" * 60, "Enjoy your trip to Rome! 🏛️", "=" * 60])
    return "\n".join(lines)
...
def render_preference_form():
    ...
    available_hours = st.sidebar.slider("Hours available", min_value=2.0, max_value=12.0, value=8.0, step=0.5)
    max_budget = st.sidebar.number_input("Maximum budget (EUR)", min_value=0.0, max_value=500.0, value=100.0, step=10.0)
    max_walking_km = st.sidebar.slider("Max walking distance (km)", min_value=2.0, max_value=20.0, value=10.0, step=1.0)
    crowd_tolerance = st.sidebar.radio(
        "Crowd preference",
        options=["avoid", "neutral", "dont_care"],
        format_func=lambda x: {"avoid": "😰 Avoid crowds", "neutral": "😐 Neutral", "dont_care": "😎 Don't care"}[x],
        index=1
    )
    start_hour = st.sidebar.slider("Start time", min_value=6, max_value=14, value=9, format="%d:00")
    start_time = time(hour=start_hour, minute=0)
    
    return UserPreferences(
        interests=interests, available_hours=available_hours, max_budget=max_budget,
        max_walking_km=max_walking_km, crowd_tolerance=crowd_tolerance, start_time=start_time
    )
